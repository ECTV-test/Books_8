...
if(route.name === "details"){
  ...
  if(route.name === "reader"){
    ...
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      ...
      showPlayer();
      let idx;
      const __r = state.route || route || {};
      const __startIndex = Number(__r.startIndex);
      const __startPara  = Number(__r.startPara);
      const __force = !!__r.forceStartIndex;

      if(__force && Number.isFinite(__startIndex)){
        idx = __startIndex;
      } else if(Number.isFinite(__startPara)){
        idx = __startPara;
      } else if(Number.isFinite(__startIndex) && __startIndex>0){
        idx = __startIndex;
      } else {
        idx = (function(){ try{ return restoreReadingProgress()||0; }catch(e){ return 0; } })();
      ...
      setCursorIndex(idx, {syncUI:true, scroll:true});
      if((state.route||route||{}).autoPlay){
        setTimeout(()=>{ try{ startReading(); }catch(e){} }, 50);
      }
    });
  }
  ...
  if(tab==="bookmarks"){
    app.querySelectorAll("[data-bm-play]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        // Play = open Listen mode and start from bookmark line
    ...
    app.querySelectorAll("[data-bm-go]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        const it = list.find(x=>x && x.id===entryId);
        const idx = Number(it?.paraIdx ?? 0);
        // Go = open Read mode and jump to bookmark line with highlight
