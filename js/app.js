...
if(route.name === "details"){
  ...
  if(route.name === "reader"){
    ...
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      ...
      setCursorIndex(idx, {syncUI:true, scroll:true});
      // Autoplay from bookmarks (Play)
      try{
        if(state.route?.autoPlay){
          const __ap = !!state.route.autoPlay;
          state.route.autoPlay = false;
          if(__ap) setTimeout(()=>{ try{ startReading(); }catch(e){} }, 60);
        }
      }catch(e){}
...
function renderLibrary(){
  ...
  if(tab==="bookmarks"){
    function _bmOpen(it, play){
      if(!it) return;
      const src = String(it.sourceLang || state.reading?.sourceLang || state.book?.sourceLang || "en").trim().toLowerCase();
      const trg = String(it.targetLang || state.reading?.targetLang || "uk").trim().toLowerCase();
      const lv  = normalizeLevel(it.level || state.reading?.level || "original");
      const idx = Number.isFinite(Number(it.lineIndex)) ? Number(it.lineIndex) : Number(it.paraIdx||0);

      try{ state.reading.sourceLang = src; }catch(e){}
      try{ state.reading.targetLang = trg; }catch(e){}
      try{ state.reading.level = lv; }catch(e){}

      // Go -> open Read (bireader) to show highlight immediately
      // Play -> open Listen (reader) and autostart from the bookmark line
      if(play){
        go({name:"reader", bookId, startIndex: idx, forceStartIndex:true, autoPlay:true}, {push:true});
      }else{
        go({name:"bireader", bookId, startIndex: idx, forceStartIndex:true}, {push:true});
      }
    }
