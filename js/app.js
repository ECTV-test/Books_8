...
function addBookmarkEntry({bookId, paraIdx, raw, tr, sourceLang, targetLang, mode, level}){
  if(!bookId) return;
  ...
  const entry = {
    ...
    tr: t,
    createdAt: Date.now(),
    sourceLang: sourceLang ? String(sourceLang).toLowerCase() : undefined,
    targetLang: targetLang ? String(targetLang).toLowerCase() : undefined,
    mode: mode ? String(mode).toLowerCase() : undefined,
    level: level ? String(level).toLowerCase() : undefined
  };
...
// ------------------------------
// Bookmarks Sheet (per-book, quick UI from Details)
// ------------------------------
let bookmarksSheetEl = null;

function ensureBookmarksSheet(){
  if(bookmarksSheetEl) return bookmarksSheetEl;
  try{
    const el = document.createElement("div");
    el.id = "bookmarksSheet";
    el.className = "sheet";
    ...
    el.innerHTML = `
      ...
      <div class="bmSheetTop">
        ...
        <div class="bmSheetActions">
          <button class="miniBtn" id="bmSheetAdd" title="Add">＋</button>
          <button class="miniBtn" id="bmSheetAll" title="All">All</button>
          <button class="miniBtn" id="bmSheetClose" title="Close">✕</button>
        </div>
      </div>
      <div class="bmSheetList" id="bmSheetList"></div>
    `;
    document.body.appendChild(el);
    bookmarksSheetEl = el;

    // close
    el.querySelector("#bmSheetClose").addEventListener("click", (e)=>{ e.preventDefault(); e.stopPropagation(); closeSheet(bookmarksSheetEl); });
    // backdrop click closes
    try{
      if(sheetBackdrop){
        sheetBackdrop.addEventListener("click", ()=>{ try{ if(bookmarksSheetEl?.classList.contains("open")) closeSheet(bookmarksSheetEl); }catch(e){} });
      }
    }catch(e){}
  }catch(e){}
  return bookmarksSheetEl;
}

function normalizeLevelSafe(x){
  try{ return normalizeLevel(x); }catch(e){}
  return String(x||"original").toLowerCase();
}

function openBookBookmarksSheet(bookId){
  try{
    if(!bookId) return;
    ensureBookmarksSheet();
    renderBookBookmarksSheet(bookId);
    openSheet(bookmarksSheetEl);
  }catch(e){}
}

function renderBookBookmarksSheet(bookId){
  const el = ensureBookmarksSheet();
  const listEl = el.querySelector("#bmSheetList");
  if(!listEl) return;

  const items = loadBookmarks(bookId) || [];
  const b = state.catalog?.find(x=>x.id===bookId) || state.book || {};
  const title = (b.title_en || b.title_ua || "Bookmarks");
  try{ el.querySelector(".bmSheetTitle").textContent = title; }catch(e){}

  // wire top actions each time (bookId changes)
  const btnAdd = el.querySelector("#bmSheetAdd");
  const btnAll = el.querySelector("#bmSheetAll");
  if(btnAdd){
    btnAdd.onclick = ()=>{
      try{
        const idx = getCursorIndex();
        const rawLine = String((state.book?.text || [])[idx] || "").trim();
        const sLang = String(state.reading?.sourceLang || state.book?.sourceLang || "en").toLowerCase();
        const tLang = String(state.reading?.targetLang || "uk").toLowerCase();
        const lv = normalizeLevelSafe(core?.getState?.()?.level || state.reading?.level || "original");
        const m = pkgMode(state.route?.name);
        addBookmarkEntry({bookId, paraIdx: idx, raw: rawLine, tr:"", sourceLang:sLang, targetLang:tLang, mode:m, level:lv});
        renderBookBookmarksSheet(bookId);
      }catch(e){}
    };
  }
    ...
    btnAll.onclick = ()=>{
      try{
        ...
        try{
          ...
          state.ui.backToBook = {
            bookId,
            sourceLang: String(s.src || state.reading?.sourceLang || "en").toLowerCase(),
            targetLang: String(s.trg || state.reading?.targetLang || "uk").toLowerCase(),
            level: normalizeLevelSafe(s.level || state.reading?.level || "original"),
            lineIndex: Number.isFinite(Number(s.lineIndex)) ? Number(s.lineIndex) : getCursorIndex(),
            mode: String(s.mode || "read").toLowerCase(),
            ts: Date.now()
          };
        }catch(e){}
        closeSheet(bookmarksSheetEl);
  ...
  if(!items.length){
    listEl.innerHTML = `<div class="bmSheetEmpty">No bookmarks yet. Tap ＋ to add one.</div>`;
    return;
  }

  listEl.innerHTML = items.map((it)=>{
    const idx = Number(it.paraIdx||0);
    const lv = normalizeLevelSafe(it.level || "original");
    const s = String(it.sourceLang||"").toLowerCase();
    const t = String(it.targetLang||"").toLowerCase();
    const m = String(it.mode||"").toLowerCase();
    const modeLabel = (m==="listen") ? "Listen" : "Read";
    const raw = String(it.raw||"").trim();
    const preview = raw ? escapeHtml(raw.slice(0, 80)) : `Line ${idx+1}`;
    return `
      <div class="bmSheetRow" data-bm="${escapeHtml(it.id)}">
        <div class="bmSheetLeft">
          <div class="bmSheetMeta">${escapeHtml(lv)} • ${escapeHtml((s||"").toUpperCase())}→${escapeHtml((t||"").toUpperCase())} • ${modeLabel} • ${idx+1}</div>
          <div class="bmSheetText">${preview}</div>
        </div>
        <div class="bmSheetBtns">
          <button class="miniBtn" data-act="go">Go</button>
          <button class="miniBtn" data-act="play">Play</button>
          <button class="miniBtn danger" data-act="del">Del</button>
        </div>
      </div>
    `;
  }).join("");
  ...
  // row actions
  listEl.querySelectorAll("[data-bm]").forEach(row=>{
    const id = row.getAttribute("data-bm");
    ...
    row.querySelectorAll("button[data-act]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        if(act==="del"){
          removeBookmarkEntry(bookId, it.id);
          renderBookBookmarksSheet(bookId);
          return;
        }
        const idx = Number(it.paraIdx||0);
        ...
        const lv = normalizeLevelSafe(it.level || core?.getState?.()?.level || "original");
        try{
          // set current selection to match bookmark
          state.reading = state.reading || {};
          state.reading.sourceLang = s;
          state.reading.targetLang = t;
          state.reading.level = lv;
          if(core && typeof core.switchLangPair==="function"){
            core.switchLangPair(s, t, lv);
          }
        }catch(e){}
        closeSheet(bookmarksSheetEl);
        // route mapping: reader = listen, bireader = read
        const routeName = (act==="play") ? "reader" : "bireader";
...
`;
  ...
  document.getElementById("detailsBookmark").onclick = ()=>openBookBookmarksSheet(state.book?.id || state.route?.bookId);
...
function closeSheet(el){
  ...
  setTimeout(()=>{
    ...
    const anyOpen = (settings && settings.classList.contains('open')) || (devPanel && devPanel.classList.contains('open')) || (chaptersSheet && chaptersSheet.classList.contains('open')) || (bookmarksSheetEl && bookmarksSheetEl.classList.contains('open'));
