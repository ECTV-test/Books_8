...
function jumpToChapter(chIdx){
  ...
  if(state.route?.name === 'reader' || state.route?.name === 'bireader'){
    ...
    // Jump inside active reading screen
      ...
      // If we arrived from a bookmark targeting a specific word, highlight that word
      try{
...
async function showLineCard(paraIdx){
  ...
  try{
    ...
    const srcLang = String(state.reading?.sourceLang || b.sourceLang || "en").trim().toLowerCase();
