...
function renderCatalog(){
  ...
  // v8: Continue Reading meta lines (include level + pkg + %)
  contLevelLabel = "Original";
  contMeta1 = "";
  contMeta2 = "";
  try{
    if(cont){
      const last = getLastPkg(cont.id);
      if(last && last.level){
        const lv = normalizeLevel(last.level);
        contLevelLabel = (lv === "original") ? "Original" : String(lv).toUpperCase();
      }
      contMeta1 = ["NEW", contLevelLabel].filter(Boolean).join(" • ");
      const pctTxt = `${Math.round(contShowPct)}%`;
      // Keep it clean: no "Resume" word and no play icon here.
      // Level is already shown in meta1.
      contMeta2 = contShowLabel ? `${contShowLabel} • ${pctTxt}` : `${pctTxt}`;
    }
  }catch(e){}
}catch(e){}

  // Continue Reading meta (hoisted)
                ...
                ${pkgs && pkgs.length ? `
                  <div class="pkgRow">
                    ${pkgs.map(x=>{
                      ...
                      // No "Resume" word / play icon in chips. Highlight resume chip via CSS only.
                      const lead = lvLabel;
