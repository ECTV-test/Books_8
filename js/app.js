...
function switchMode(nextRoute){
  ...
  try{
    ...
    if(mode === "reader"){
      ...
      startIndex = Number.isFinite(a) ? a : (Number.isFinite(r) ? r : (Number.isFinite(openaiLineIndex) ? openaiLineIndex : 0));
...
/* ---------------------------
   Bookmarks v8 (sheet + pkg-aware)
--------------------------- */
function normalizeLevel(lv){
  const x = String(lv||"original").toLowerCase();
  if(x==="a1"||x==="a2"||x==="b1"||x==="original") return x;
  return "original";
}
function getCurrentPkgCtx(){
  let bookId = state.book?.id || state.route?.bookId || state.route?.id || "";
  let src = String(state.reading?.sourceLang || state.book?.sourceLang || "en").toLowerCase();
  let trg = String(state.reading?.targetLang || "uk").toLowerCase();
  let level = normalizeLevel(state.reading?.level || "original");
  let mode = pkgMode(state.route?.name);
  let lineIndex = 0;
  try{
    if(window.core && window.core.getState){
      const st = window.core.getState();
      if(st){
        bookId = st.bookId || bookId;
        src = String(st.src||src).toLowerCase();
        trg = String(st.trg||trg).toLowerCase();
        level = normalizeLevel(st.level||level);
        mode = String(st.mode||mode);
        if(typeof st.lineIndex==="number") lineIndex = Number(st.lineIndex||0);
      }
    }
  }catch(e){}
  return {bookId:String(bookId||""), src, trg, level, mode, lineIndex};
}
function migrateBookmarkItem(it){
  if(!it || typeof it!=="object") return null;
  const out = {...it};
  if(typeof out.lineIndex!=="number"){
    if(typeof out.paraIdx==="number") out.lineIndex = out.paraIdx;
    else out.lineIndex = 0;
  }
  if(!out.level) out.level = normalizeLevel(out.level||"original");
  if(out.sourceLang) out.sourceLang = String(out.sourceLang).toLowerCase();
  if(out.targetLang) out.targetLang = String(out.targetLang).toLowerCase();
  if(!out.mode) out.mode = "read";
  if(!out.createdAt) out.createdAt = Date.now();
  if(!out.id) out.id = `${Date.now()}_${Math.random().toString(16).slice(2)}`;
  return out;
}
function listBookmarksForBook(bookId){
  const arr = loadBookmarks(bookId).map(migrateBookmarkItem).filter(Boolean);
  // keep original order (old requirement: append new under)
  return arr;
}
function addBookmarkAtCurrentLine(){
  const ctx = getCurrentPkgCtx();
  if(!ctx.bookId) return;
  // try to pull excerpt from book text
  let excerpt = "";
  try{
    const lines = state.book?.text || [];
    excerpt = String(lines[ctx.lineIndex] || "").trim();
  }catch(e){}
  addBookmarkEntry({
    bookId: ctx.bookId,
    paraIdx: ctx.lineIndex,
    raw: excerpt,
    tr: "",
    sourceLang: ctx.src,
    targetLang: ctx.trg,
...
function openBookBookmarksSheet(){
  const ctx = getCurrentPkgCtx();
  if(!ctx.bookId) return;
  const book = state.book;
  const items = listBookmarksForBook(ctx.bookId);
  // build UI
  const overlay = document.createElement("div");
  overlay.className = "bmSheetOverlay";
  overlay.innerHTML = `
    <div class="bmSheet" role="dialog" aria-modal="true">
      <div class="bmSheetTop">
        <div class="bmSheetTitle">Bookmarks</div>
        <div class="bmSheetActions">
          <button class="bmMiniBtn" id="bmAddNow" title="Add bookmark">＋</button>
          <button class="bmMiniBtn" id="bmAll" title="All bookmarks">All</button>
          <button class="bmMiniBtn" id="bmClose" title="Close">✕</button>
        </div>
      </div>
      <div class="bmSheetBody">
        ${items.length ? items.map((it, idx)=>{
          const lvl = normalizeLevel(it.level||"original");
          const s = String(it.sourceLang||ctx.src||"en").toLowerCase();
          const t = String(it.targetLang||ctx.trg||"uk").toLowerCase();
          const m = String(it.mode||"read");
          const pct = (()=>{ try{
            const total = Number(state.book?.text?.length||0)||0;
            if(!total) return "";
            return Math.max(0, Math.min(100, Math.round((Number(it.lineIndex||0)/Math.max(1,total))*100))) + "%";
          }catch(e){ return ""; } })();
          const label = `${lvl.toUpperCase()} • ${s.toUpperCase()}→${t.toUpperCase()} • ${m==="listen"?"Listen":"Read"} ${pct}`;
          const raw = String(it.raw||"").trim();
          return `
            <div class="bmRow" data-bm-row>
              <div class="bmRowMain">
                <div class="bmRowLabel">${escapeHtml(label)}</div>
                <div class="bmRowText">${escapeHtml(raw || ("Line " + (Number(it.lineIndex||0)+1)))}</div>
              </div>
              <div class="bmRowBtns">
                <button class="bmBtn primary" data-bm-go="${escapeHtml(ctx.bookId)}::${escapeHtml(it.id)}">Go</button>
                <button class="bmBtn" data-bm-play="${escapeHtml(ctx.bookId)}::${escapeHtml(it.id)}">Play</button>
                <button class="bmBtn" data-bm-del="${escapeHtml(ctx.bookId)}::${escapeHtml(it.id)}">Del</button>
              </div>
            </div>
          `;
        }).join("") : `<div class="bmEmpty">No bookmarks yet. Tap ＋ to add one.</div>`}
      </div>
    </div>
  `;
  function close(){
  ...
  const btnClose = overlay.querySelector("#bmClose");
  const btnAll = overlay.querySelector("#bmAll");
  const btnAdd = overlay.querySelector("#bmAddNow");
  if(btnClose) btnClose.onclick = close;
  if(btnAdd) btnAdd.onclick = ()=>{ addBookmarkAtCurrentLine(); close(); openBookBookmarksSheet(); };

  if(btnAll){
    btnAll.onclick = ()=>{
      // remember return point (last stop) and open Library > Bookmarks
      try{
        const st = (window.core && window.core.getState) ? window.core.getState() : null;
        const ret = {
          bookId: ctx.bookId,
          src: ctx.src,
          trg: ctx.trg,
          level: ctx.level,
          mode: ctx.mode,
          lineIndex: st && typeof st.lineIndex==="number" ? Number(st.lineIndex||0) : ctx.lineIndex
        };
        state.ui = state.ui || {};
        state.ui.bookmarksReturn = ret;
  ...
  // wire buttons
  overlay.querySelectorAll("[data-bm-go]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const key = btn.getAttribute("data-bm-go")||"";
      const [bid, id] = key.split("::");
      const list = listBookmarksForBook(bid);
      const it = list.find(x=>x && x.id===id);
      if(!it) return;
      close();
      // Go = open read at bookmark
      const lvl = normalizeLevel(it.level||"original");
      const s = String(it.sourceLang||ctx.src).toLowerCase();
      const t = String(it.targetLang||ctx.trg).toLowerCase();
      const li = Number(it.lineIndex||0);
      try{
        if(window.core){
          window.core.switchLangPair(s, t, lvl);
          window.core.switchMode("read");
          window.core.setLine(li);
          window.core.saveProgress();
        }
      }catch(e){}
      if(state.route?.name!=="reader" && state.route?.name!=="bireader"){
        go({name:"reader", bookId: bid}, {push:true});
      }else{
        setCursorIndex(li, {syncUI:true, scroll:true});
      }
    });
  });
  overlay.querySelectorAll("[data-bm-play]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const key = btn.getAttribute("data-bm-play")||"";
      const [bid, id] = key.split("::");
      const list = listBookmarksForBook(bid);
      const it = list.find(x=>x && x.id===id);
      if(!it) return;
      ...
      try{ startReading(); }catch(e){}
    });
  });
  overlay.querySelectorAll("[data-bm-del]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const key = btn.getAttribute("data-bm-del")||"";
      const [bid, id] = key.split("::");
      removeBookmarkEntry(bid, id);
      close();
      openBookBookmarksSheet();
    });
  });
}
}
      ...
      ${cont ? `
        ...
        <div class="cardWide" id="continueCard" role="button" tabindex="0">
          ...
          <div class="info">
            ...
            <p class="meta">${escapeHtml((cont.author||cont.series||""))}${cont.level ? ` • ${escapeHtml(cont.level)}` : ``}${contShowLabel?` • ${escapeHtml(contShowLabel)}`:``}</p>
          </div>
  ...
  app.innerHTML = `
    <div class="wrap">
      ...
      <div class="libraryList">
        ${tab==="bookmarks" ? (
          ((state.ui&&state.ui.bookmarksReturn&&state.ui.bookmarksReturn.bookId)?`<div class="bmBackWrap"><button class="bmBackToBook" id="bmBackToBook">↩︎ Back to book</button></div>`:"") +
                ...
                ${pkgs && pkgs.length ? `
                  <div class="pkgRow">
                    ${pkgs.map(x=>{
                      ...
                      return `<span class="pkgChip">${flagFor(s)} ${s.toUpperCase()} <span class="arrow">→</span> ${flagFor(t)} ${t.toUpperCase()} <span class="mode">${modeLabel}</span> <span class="pct">${pct}%</span></span>`;
                    }).join("")}
                  </div>
                ` : ``}
              </div>
...
// Back to book (only if opened Bookmarks from a book)
const __bb = document.getElementById("bmBackToBook");
if(__bb){
  __bb.addEventListener("click", (e)=>{
    e.preventDefault();
    e.stopPropagation();
    const ret = state.ui && state.ui.bookmarksReturn ? state.ui.bookmarksReturn : null;
    if(!ret || !ret.bookId) return;
    // restore package + position
    try{
      const lvl = normalizeLevel(ret.level||"original");
      const s = String(ret.src||ret.sourceLang||"en").toLowerCase();
      const t = String(ret.trg||ret.targetLang||"uk").toLowerCase();
      const li = Number(ret.lineIndex||0);
      state.reading = state.reading || {};
      state.reading.level = lvl;
      state.reading.sourceLang = s;
      state.reading.targetLang = t;
      try{
        if(window.core){
          window.core.switchLangPair(s, t, lvl);
          window.core.switchMode(String(ret.mode||"read"));
          window.core.setLine(li);
          window.core.saveProgress();
        }
      }catch(_e){}
      state.ui.bookmarksReturn = null;
      go({name:"reader", bookId: String(ret.bookId), startPara: li}, {push:true});
    }catch(err){}
  });
}
...
app.querySelectorAll("[data-bm-play]").forEach(btn=>{
  btn.addEventListener("click", (e)=>{
    ...
    const list = listBookmarksForBook(bookId);
    const it = list.find(x=>x && x.id===entryId);
    if(!it) return;
    const lvl = normalizeLevel(it.level||"original");
    const s = String(it.sourceLang||state.reading?.sourceLang||"en").toLowerCase();
    const t = String(it.targetLang||state.reading?.targetLang||"uk").toLowerCase();
    const li = Number(it.lineIndex||it.paraIdx||0);
    state.reading = state.reading || {};
    state.reading.level = lvl;
    state.reading.sourceLang = s;
    state.reading.targetLang = t;
    ...
    go({name:"reader", bookId, startPara: li}, {push:true});
    // start after render
    setTimeout(()=>{ try{ startReading(); }catch(e){} }, 100);
  });
});
    ...
    <div class="detailsGrid">
      ...
      <div>
        <div>
          ...
          <p class="detailsMeta">${escapeHtml((b.author||""))}${b.level?` • ${escapeHtml(String(b.level))}`:``}${pagesEst?` • ~${escapeHtml(String(pagesEst))} pages`:``}${savedPct||savedPct===0?` • ${escapeHtml(String(Math.round(savedPct)))}%`:``}${savedLabel?` • ${escapeHtml(savedLabel)}`:``}</p>
