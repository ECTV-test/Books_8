...
function addBookmarkFromPopover(){
  try{
    ...
    addBookmarkEntry({bookId, paraIdx: popCtx.paraIdx || 0, raw, tr, wordIdx: popCtx.wordIdx, sourceLang: sLang, targetLang: tLang, mode: m, level: state.reading?.level || ""});
...
function jumpToChapter(chIdx){
  ...
  if(state.route?.name === 'reader' || state.route?.name === 'bireader'){
    ...
    setCursorIndex(idx, {syncUI:true, scroll:true});
      // Bookmark word focus
      try{
        const wIdx = Number(state.route?.wordIdx);
        if(Number.isFinite(wIdx) && wIdx>=0){
          setTimeout(()=>{
            try{ setActiveParaWord(idx, wIdx); }catch(e){}
            try{
              const sp = state.reading?.paraWords?.[idx]?.[wIdx];
              if(sp && sp.scrollIntoView){ sp.scrollIntoView({block:"center", inline:"nearest"}); }
            }catch(e){}
          }, 60);
        }
      }catch(e){}
      try{ state.route.wordIdx = -1; }catch(e){}
...
function addBookmarkEntry({bookId, paraIdx, raw, tr, wordIdx, sourceLang, targetLang, mode, level}){
  if(!bookId) return;
  ...
  const entry = {
    ...
    wordIdx: Number.isFinite(wordIdx) && Number(wordIdx) >= 0 ? Number(wordIdx) : -1,
  ...
  if(tab==="bookmarks"){
    ...
    app.querySelectorAll("[data-bm-go]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        go({name:"reader", bookId, startIndex: idx, forceStartIndex:true, wordIdx: (Number.isFinite(it?.wordIdx) && Number(it.wordIdx)>=0) ? Number(it.wordIdx) : -1});
      });
    });
...
async function showTranslation(span){
  ...
  try{
    ...
    popCtx = { bookId: state.book?.id || state.route?.bookId, paraIdx: Number.isFinite(pIdx) ? pIdx : 0, raw, tr: "", wordIdx: (()=>{ try{ const list = state.reading?.paraWords?.[Number.isFinite(pIdx)?pIdx:0] || []; return list.indexOf(span); }catch(e){ return -1; } })() };
