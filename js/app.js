...
function renderReader(){
  ...
  try{
    if(state.route && state.route.startPara != null){
      ...
      // Jump to paragraph (single source of truth)
      setCursorIndex(sp, {syncUI:true, scroll:true});
      // consume once
...
function setActivePara(idx){
  // v8: cursor is managed by core/setCursorIndex(); this function only paints UI.
...
btnStart.onclick = ()=>{
  ...
  if(state.route?.name==="reader"){
    state.reading.resumeIndexReader = 0;
    state.reading.activeParaIndex = 0;
    setCursorIndex(0, {syncUI:true, scroll:false});
...
tSwap.onclick = ()=>{ state.reading.swapLang = !state.reading.swapLang; toggleUI(tSwap, state.reading.swapLang); if(state.route?.name==="bireader"){ document.body.classList.toggle("swapLang", !!state.reading.swapLang); setCursorIndex(getCursorIndex(), {syncUI:true, scroll:false}); } };
  ...
  if(!state.reading.highlight){
    ...
    if(mode === "reader"){
      if(state.reading.activeParaIndex != null && state.reading.activeParaIndex >= 0){
        setCursorIndex(getCursorIndex(), {syncUI:true, scroll:false});
      }
    }
    if(mode === "bireader"){
