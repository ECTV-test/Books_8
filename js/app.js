...
function addBookmarkEntry({bookId, paraIdx, raw, tr, lineIndex, level, sourceLang, targetLang, mode, wordIndex, wordKey}){
  ...
  // filter for current context (level + lang pair + mode)
  const ctxLevel = _bmGetLevel();
  const ctxPair = _bmGetLangPair();
  const ctxMode = _bmGetMode();
  const list = (_all||[]).filter(it=>{
    if(!it) return false;
    const lvl = String(it.level||"original");
    const src = String(it.sourceLang||ctxPair.src||"en");
    const trg = String(it.targetLang||ctxPair.trg||"uk");
    const md  = String(it.mode||"read");
    return lvl===String(ctxLevel||"original") && src===String(ctxPair.src||"en") && trg===String(ctxPair.trg||"uk") && md===String(ctxMode||"read");
  });
  const entry = {
          ...
          (bookmarksGroups.length ? bookmarksGroups.map(({b,items})=>`
            <div class="bmBook">
              <div class="bmHead" data-open="${escapeHtml(b.id)}">
                ...
                ${tab==="bookmarks" && state.ui?.backToBook && state.ui.backToBook.bookId===b.id ? `
                  <button class="bmBackMini" id="backToBookBtn" title="Back to book">↩︎</button>
                ` : ``}
              </div>
              <div class="bmItems">
                ${(()=>{ 
                    ...
                    ${g.items.map((it, idx)=>`
                      <div class="bmItem" data-bm-item>
                        <div class="bmMain">
                          ...
                          <p class="bmRaw">${escapeHtml((it.raw||it.tr||"").slice(0,140))}</p>
                        </div>
                        <div class="bmBtns">
