...
function showBookBookmarksSheet(bookId){
  ...
  wrap.querySelector("#bmAddBtn").onclick = ()=>{ addBookmarkHere(bookId); try{ if((state.route?.name==='reader' || state.route?.name==='bireader') && String(state.book?.id||'')===String(bookId)){ applyBookmarkMarksInDom(); } }catch(e){} close(); showBookBookmarksSheet(bookId); };
  ...
  wrap.querySelectorAll("[data-bm-del]").forEach(btn=>{
    btn.onclick = ()=>{
      ...
      try{ if((state.route?.name==='reader' || state.route?.name==='bireader') && String(state.book?.id||'')===String(bid)){ applyBookmarkMarksInDom(); } }catch(e){}
      close();
...
// ===== Bookmark marks (tiny indicators in text) =====
function _bmBuildMarkCache(bookId){
  try{
    const list = loadBookmarks(bookId) || [];
    ...
    list.forEach(it=>{
      if(!it) return;
      const li = Number.isFinite(Number(it.lineIndex)) ? Number(it.lineIndex) : (Number.isFinite(Number(it.paraIdx)) ? Number(it.paraIdx) : null);
      if(li==null || !Number.isFinite(li)) return;
      lineSet.add(li);
    ...
    state.reading.bmLineSet = lineSet;
    state.reading.bmWordMap = wordMap;
  }catch(e){
    state.reading.bmLineSet = new Set();
    state.reading.bmWordMap = new Map();
  }
}
function applyBookmarkMarksInDom(){
  try{
    const bookId = state.book?.id || state.route?.bookId;
    if(!bookId) return;
    _bmBuildMarkCache(bookId);

    // clear previous
    try{ document.querySelectorAll(".bmWordMark").forEach(el=>el.classList.remove("bmWordMark")); }catch(e){}

    // line marks
    try{
      document.querySelectorAll("[data-bm-line]").forEach(el=>{
        const li = Number(el.dataset.bmLine);
        const on = !!state.reading.bmLineSet && state.reading.bmLineSet.has(li);
    ...
    // word marks (if any)
    try{
      const wm = state.reading.bmWordMap || new Map();
      for(const [li, set] of wm.entries()){
        if(!set) continue;
        const words = (state.reading.paraWords?.[li] && state.reading.paraWords[li].length)
          ? state.reading.paraWords[li]
          : [...document.querySelectorAll(`.para[data-para="${li}"] .w[data-token="word"]`)];
        for(const wi of set){
          const wEl = words[Number(wi)];
          if(wEl) wEl.classList.add("bmWordMark");
        }
      }
    }catch(e){}
  }catch(e){}
              ...
              <div class="bmItems">
                ${items.map((it, idx)=>`
                  <div class="bmItem" data-bm-item>
                    <div class="bmMain">
                      <p class="bmLabel">Закладка ${idx+1} <span class="bmLevel">${escapeHtml(String(it.level||"original").toUpperCase().replace("ORIGINAL","ORIG"))}</span> <span class="bmPair">${escapeHtml(formatPkgLabel((it.sourceLang||getLastPkg(b.id)?.sourceLang||state.reading?.sourceLang||"en"), (it.targetLang||getLastPkg(b.id)?.targetLang||state.reading?.targetLang||"uk"), (it.mode||getLastPkg(b.id)?.mode||"read")) )}</span></p>
...
function renderReader(){
        ...
        ${lines.map((p, i)=>{
          ...
          return `
            <div class="listenLine ${isCh ? "chapterLine" : ""}" data-para-wrap="${i}">
              ...
              <span class="bmLineMark" data-bm-line="${i}" aria-hidden="true"></span>
  ...
  // Quick open Bookmarks (this book only)
  ...
  // Consume one-time start position (e.g., coming from a bookmark / resume jump)
  try{
    const sp = (state.route && (state.route.startPara ?? state.route.startIndex)) ?? null;
    const sw = (state.route && (state.route.startWordIndex ?? null)) ?? null;
    if(sp !== null && sp !== undefined){
      try{ setCursorIndex(sp); }catch(e){}
      if(sw !== null && sw !== undefined){
        try{ setActiveParaWord(sp, sw); }catch(e){}
      }
      ...
      <div class="paper">
        <div class="paperInner">
          ...
          ${lines.map((ln, i)=>{
            ...
            return `
              <div class="paraLine ${isCh ? "chapterLine" : ""}"><span class="bmLineMark" data-bm-line="${i}" aria-hidden="true"></span>
