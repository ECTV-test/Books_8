...
function addBookmarkFromPopover(){
  try{
    ...
    addBookmarkEntry({bookId, paraIdx: popCtx.paraIdx || 0, lineIndex: (Number.isFinite(popCtx.lineIndex)?popCtx.lineIndex:(popCtx.paraIdx||0)), level: String(popCtx.level || state.reading?.level || "original"), raw, tr, sourceLang: popCtx.sourceLang || sLang, targetLang: popCtx.targetLang || tLang, mode: popCtx.mode || m, wordIdx: (Number.isFinite(popCtx.wordIdx)?popCtx.wordIdx:-1), wordRaw: popCtx.wordRaw || ""});
...
function jumpToChapter(chIdx){
  ...
  if(state.route?.name === 'reader' || state.route?.name === 'bireader'){
    ...
    // Jump inside active reading screen
      ...
      // Highlight bookmarked word (if any)
      try{
        const w = Number(state.route?.bookmarkWordIdx);
        if(Number.isFinite(w) && w >= 0){
          setTimeout(()=>{ try{ setActiveParaWord(idx, w); }catch(e){} }, 0);
        }
        try{ state.route.bookmarkWordIdx = -1; }catch(e){}
      }catch(e){}
...
function addBookmarkEntry({bookId, paraIdx, raw, tr, lineIndex, level, sourceLang, targetLang, mode, wordIdx, wordRaw}){
  if(!bookId) return;
  ...
  const entry = {
    ...
    wordIdx: (Number.isFinite(wordIdx) ? Number(wordIdx) : -1),
    wordRaw: String(wordRaw||"").trim(),
    raw: r,
    tr: t,
    createdAt: Date.now()
  };
...
function _bmOpenFromEntry(bookId, entry, play){
  ...
  go({name: routeName, bookId: bookId, startIndex: idx, forceStartIndex: true, autoPlay: !!play, bookmarkWordIdx: (Number.isFinite(entry.wordIdx)?Number(entry.wordIdx):-1)});
}
...
`;
  ...
  document.getElementById("detailsBookmark").onclick = ()=>showBookBookmarksSheet(route.bookId || state.route?.bookId || state.book?.id);
...
async function showTranslation(span){
  ...
  try{
    ...
    const list = (state.reading.paraWords && state.reading.paraWords[pIdx]) ? state.reading.paraWords[pIdx] : [];
    const wIdx = Array.isArray(list) ? list.indexOf(span) : -1;
    popCtx = {
      bookId: state.book?.id || state.route?.bookId,
      paraIdx: Number.isFinite(pIdx) ? pIdx : 0,
      lineIndex: Number.isFinite(pIdx) ? pIdx : 0,
      level: String(state.reading?.level || "original"),
      sourceLang: String(state.reading?.sourceLang || state.book?.sourceLang || "en").trim().toLowerCase(),
      targetLang: String(state.reading?.targetLang || "uk").trim().toLowerCase(),
      mode: String(pkgMode(state.route?.name) || "read"),
      wordIdx: Number.isFinite(wIdx) ? wIdx : -1,
      wordRaw: raw,
      raw,
      tr: ""
    };
  }catch(e){
    popCtx = { bookId: state.book?.id || state.route?.bookId, paraIdx: 0, lineIndex: 0, wordIdx: -1, wordRaw: raw, raw, tr:"" };
  }
...
async function showLineCard(paraIdx){
  ...
  try{
    ...
    const sourceLang = String(state.reading?.sourceLang || b.sourceLang || "en").trim().toLowerCase();
    const targetLang = String(state.reading?.targetLang || "uk").trim().toLowerCase();
    const mode = String(pkgMode(state.route?.name) || "read");
    popCtx = {
      bookId: b.id || state.route?.bookId,
      paraIdx: Number.isFinite(paraIdx) ? Number(paraIdx) : 0,
      lineIndex: Number.isFinite(paraIdx) ? Number(paraIdx) : 0,
      level, sourceLang, targetLang, mode,
      wordIdx: -1, wordRaw: "",
      raw, tr: ""
    };
  }catch(e){
    popCtx = { bookId: b.id || state.route?.bookId, paraIdx: 0, lineIndex: 0, wordIdx:-1, wordRaw:"", raw, tr: "" };
  }
