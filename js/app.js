      ...
      ${cont ? `
        ...
        <div class="cardWide" id="continueCard" role="button" tabindex="0">
          ...
          <div class="info">
            ...
            <p class="meta">${escapeHtml((cont.author||cont.series||"NEW"))}${contShowLabel?` • ${escapeHtml(contShowLabel)}`:``}</p>
          </div>
          ...
          (list.length ? list.map(({b,p,pkgs})=>`
            <div class="libraryItem" data-open="${escapeHtml(b.id)}">
              ...
              <div style="flex:1; min-width:0;">
                ...
                <p class="meta">${escapeHtml(getBookSubtitleForList(b))}</p>
                ${pkgs && pkgs.length ? `
                  <div class="pkgRow">
                    ${pkgs.map(x=>{
...
// Pick a short subtitle/blurb for library lists.
// Priority: blurb/subtitle/tagline -> first meaningful line of description -> category/series/author -> empty
function getBookSubtitleForList(b){
  try{
    ...
    let s = clean(b?.blurb || b?.subtitle || b?.tagline || "");
    if(!s){
      const desc = String(b?.description||"");
      const lines = desc.split(/\r?\n/).map(clean).filter(Boolean);
      const title = clean(b?.title_en || b?.title_ua || b?.title || "");
      const lvl = clean(b?.level || "");
      const filtered = lines.filter(x=>{
        const low = x.toLowerCase();
        if(title && low===title.toLowerCase()) return false;
        if(lvl && low.includes(lvl.toLowerCase()) && low.length<=50) return false;
        if(low.startsWith("level") || low.startsWith("рівень") || low.startsWith("уровень")) return false;
        return true;
      });
      s = filtered[0] || "";
    }
    if(!s){
      s = clean(b?.category || b?.series || b?.author || "");
    }
    return trunc(s, 92);
  }catch(e){
    return "";
