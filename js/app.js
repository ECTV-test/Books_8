  ...
  if(tab==="bookmarks"){
    app.querySelectorAll("[data-bm-play]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        const it = _bmFind(bookId, entryId);
...
try{ clearAllWordHighlights(); }catch(e){}
  ...
  function prefetchForIndex(idx){
    ...
    const p = (async ()=>{
      ...
      // Prevent unhandled rejections if a prefetched request fails and is never awaited.
      console.warn(err);
      return null;
    });
...
const playNext = async ()=>{
    ...
    try{
      ...
      if(pref){
        ...
        const prefBlob = await pref;
        blob = prefBlob || await fetchTtsAudioBlob(speakText, {voice, instructions, speed, noCache: state.dev.noCache});
      }else{
