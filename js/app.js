...
if(route.name === "details"){
  ...
  if(route.name === "reader"){
    stopReading({save:true});
    const __forcedStart = (route && route.startPara != null) ? Number(route.startPara) :
                          (route && route.startIndex != null) ? Number(route.startIndex) : null;
    const __autoPlay = !!(route && route.autoPlay);
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      state.book = book;
      renderReader();
      showPlayer();
      ...
      // If we came from a bookmark (or any forced start), always honor it.
      if(Number.isFinite(__forcedStart) && __forcedStart >= 0){
        idx = __forcedStart;
      }else{
      ...
      if(__autoPlay){
        setTimeout(()=>{ try{ startReading(); }catch(e){} }, 120);
      }
    });
  }
  ...
  if(tab==="bookmarks"){
    app.querySelectorAll("[data-bm-play]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        if(!it) return;
        const idx = Math.max(0, Number(it.paraIdx||0));
        // Play = open Listen (reader) from bookmark line, then autoplay
        state.reading.mode = "listen";
        go({name:"reader", bookId, startPara: idx, autoPlay: true});
      });
    });
    app.querySelectorAll("[data-bm-go]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        if(!it) return;
        const idx = Math.max(0, Number(it.paraIdx||0));
        // Go = open Read (bireader) from bookmark line
        state.reading.mode = "read";
        go({name:"bireader", bookId, startPara: idx});
      });
    });
