...
function shortLevelLabel(level){
  const l = String(level||"original").trim().toLowerCase();
  if(!l || l==="original") return "ORIG";
...
function addBookmarkFromPopover(){
  try{
    ...
    addBookmarkEntry({bookId, paraIdx: (popCtx.paraIdx||0), raw, tr, lineIndex: (popCtx.lineIndex ?? popCtx.paraIdx ?? 0), level: (popCtx.level || state.reading?.level || "original"), sourceLang: (popCtx.sourceLang || sLang), targetLang: (popCtx.targetLang || tLang), mode: (popCtx.mode || m), wordIndex: Number(popCtx.wordIndex ?? -1), wordKey: String(popCtx.wordKey||"")});
...
function addBookmarkEntry({bookId, paraIdx, raw, tr, lineIndex, level, sourceLang, targetLang, mode, wordIndex, wordKey}){
  ...
  try{ refreshBookmarkMarksInView(); }catch(e){}
}

// --- Bookmark marks (subtle dots near bookmarked lines/words) ---
let __bmLineSet = null;   // Set<number>
let __bmWordSet = null;   // Set<string> like "line:word"
function __computeBookmarkMarks(bookId, {level, src, trg}){
  try{
    const L = String(level||"original").trim().toLowerCase();
    const S = String(src||"").trim().toLowerCase();
    const T = String(trg||"").trim().toLowerCase();
    const list = loadBookmarks(bookId) || [];
    const lineSet = new Set();
    const wordSet = new Set();
    list.forEach(it=>{
      if(!it) return;
      const il = String(it.level||"original").trim().toLowerCase();
      const is = String(it.sourceLang||"").trim().toLowerCase();
      const itg = String(it.targetLang||"").trim().toLowerCase();
      if(il!==L || is!==S || itg!==T) return;
      const li = Number.isFinite(it.lineIndex) ? Number(it.lineIndex) : (Number.isFinite(it.paraIdx)?Number(it.paraIdx):0);
      lineSet.add(li);
      const wi = Number(it.wordIndex);
      if(Number.isFinite(wi) && wi>=0){
        wordSet.add(`${li}:${wi}`);
      }
    });
    return {lineSet, wordSet};
  }catch(e){
    return {lineSet:new Set(), wordSet:new Set()};
  }
}
function __prepareBookmarkMarksForRender(bookId){
  try{
    const level = state.reading?.level || "original";
    const src = state.reading?.sourceLang || state.book?.sourceLang || "en";
    const trg = state.reading?.targetLang || "uk";
    const ms = __computeBookmarkMarks(bookId, {level, src, trg});
    __bmLineSet = ms.lineSet;
    __bmWordSet = ms.wordSet;
  }catch(e){
    __bmLineSet = null; __bmWordSet = null;
  }
}
function refreshBookmarkMarksInView(){
  try{
    if(!state.route || (state.route.name!=="reader" && state.route.name!=="bireader")) return;
    const bookId = state.book?.id || state.route?.bookId;
    if(!bookId) return;
    __prepareBookmarkMarksForRender(bookId);

    // Lines (paragraphs)
    document.querySelectorAll(".para[data-para]").forEach(p=>{
      const idx = Number(p.dataset.para);
      const has = __bmLineSet && __bmLineSet.has(idx);
      let dot = p.querySelector(".bmDot");
      if(has){
        if(!dot){
    ...
    // Words
    document.querySelectorAll('.para[data-para] .w[data-token="word"]').forEach(sp=>{
      const p = sp.closest(".para");
      const pi = p ? Number(p.dataset.para) : -1;
      if(pi<0) return;
      const wi = Number(sp.dataset.wi);
      const key = `${pi}:${wi}`;
      const has = __bmWordSet && __bmWordSet.has(key);
      if(has) sp.classList.add("bmMark");
      else sp.classList.remove("bmMark");
    });
  }catch(e){}
}
...
function showBookBookmarksSheet(bookId){
  bookId = bookId || state.book?.id || state.route?.bookId;
  if(!bookId) return;
  const b = getBookMeta(bookId) || state.book || {};
  const list = loadBookmarks(bookId);
  // basic modal
    ...
    <div class="bmSheet">
      <div class="bmSheetTop">
        ...
        <div class="bmSheetBtns">
          ...
          <button class="bmSheetBtn" id="bmAllBtn" title="All bookmarks">All</button>
        ...
        ${list.length ? list.map((it,i)=>`
          <div class="bmSheetItem">
            <div class="bmSheetMain">
              <div class="bmSheetLine">#${i+1} <span class="bmSheetPkg">${escapeHtml((it.level||"original").toString())} • ${escapeHtml(formatPkgLabel((it.sourceLang||_bmGetLangPair().src),(it.targetLang||_bmGetLangPair().trg),(it.mode||"read")))}</span></div>
              <div class="bmSheetRaw">${escapeHtml((it.raw||"").slice(0,140))}</div>
            </div>
  ...
  wrap.querySelectorAll("[data-bm-play]").forEach(btn=>{
    btn.onclick = ()=>{
      ...
      const entry = _bmFind(bid, eid);
      close();
      _bmOpenFromEntry(bid, entry, true);
    };
  });
...
function renderCatalog(){
  ...
  if(cont){
    ...
    const fallbackLabel = (() => {
      try{
        if(last && last.sourceLang && last.targetLang){
          const modeTxt = (last.mode && String(last.mode).toLowerCase()==="listen") ? "Listen" : "Read";
    ...
    contMeta2 = labelTxt ? `${contLevelLabel} • ${labelTxt} • ${pctTxt}` : `${contLevelLabel} • ${pctTxt}`;
  }
...
function renderLibrary(){
  ...
  app.innerHTML = `
    <div class="wrap">
      ...
      ${tab==="bookmarks" && state.ui?.backToBook ? `
        <div style="padding:0 18px 10px 18px;">
          <button class="backToBookBtn" id="backToBookBtn">↩︎ Back to book</button>
        </div>
      ` : ``}
              ...
              <div class="bmItems">
                ${items.map((it, idx)=>`
                  <div class="bmItem" data-bm-item>
                    <div class="bmMain">
                      <p class="bmLabel">Закладка ${idx+1} ${escapeHtml(shortLevelLabel(it.level))} <span class="bmPair">${escapeHtml(formatPkgLabel((it.sourceLang||getLastPkg(b.id)?.sourceLang||state.reading?.sourceLang||"en"), (it.targetLang||getLastPkg(b.id)?.targetLang||state.reading?.targetLang||"uk"), (it.mode||getLastPkg(b.id)?.mode||"read")) )}</span></p>
  ...
  if(__btb){
    __btb.onclick = ()=>{
      ...
      const routeName = (String(ctx.mode||"read")==="listen") ? "bireader" : "reader";
      go({name: routeName, bookId: ctx.bookId, startIndex: Number(ctx.lineIndex||0), forceStartIndex:true});
    };
  }
    ...
    app.querySelectorAll("[data-bm-go]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        try{
          ...
          const level = normalizeLevel(it?.level || state.reading?.level || "original");
          const prev = getPkgProgress(bookId, src, trg, level);
...
function renderReader(){
  ...
  setTheme
  try{ __prepareBookmarkMarksForRender(b.id); }catch(e){}
(state.reading.night);
  syncSettingsUI();
  applyHighlightTheme();
  ...
  if(__tbm){
    __tbm.addEventListener("click", (e)=>{
      e.preventDefault();
      e.stopPropagation();
      try{ showBookBookmarksSheet(b.id); }catch(err){}
    });
  }
...
function renderParagraph(text, pIndex, isHeadingOverride=false){
  ...
  const __hasLineBm = (__bmLineSet && __bmLineSet.has(pIndex));
  let __wi = -1;
  ...
  return `
    <p class="para" data-para="${pIndex}" style="${isHeading ? "font-weight:950;letter-spacing:.2px" : ""}">
      ${tokens.map((t)=>{
        ...
        __wi += 1;
        const __bm = (__bmWordSet && __bmWordSet.has(`${pIndex}:${__wi}`));
        return `<span class="w ${__bm ? "bmMark" : ""}" data-token="word" data-wi="${__wi}" data-key="${escapeHtml(key)}" data-raw="${escapeHtml(raw)}">${escapeHtml(raw)}</span>`;
      }).join("")}
      ${__hasLineBm ? `<span class="bmDot" aria-hidden="true"></span>` : ``}
    </p>
  `;
}
  ...
  try{
    if(save){
      ...
      if(lock && state.book && String(state.book.id||"") === String(lock.bookId||"")){
        ...
        const lv = normalizeLevel(state.reading?.level||"original");
        if(String(lock.src||"").toLowerCase() === s && String(lock.trg||"").toLowerCase() === t && normalizeLevel(lock.level||"original") === lv){
...
function _maybeShowBookmarkPlayChoice(){
  try{
    ...
    // Build a tiny modal (no CSS regressions)
    const wrap = document.createElement("div");
    ...
    wrap.innerHTML = `
      <div style="width:min(440px, calc(100vw - 36px));background:#fff;border-radius:18px;padding:16px 16px 14px;box-shadow:0 18px 60px rgba(0,0,0,.25);">
        <div style="font-weight:900;font-size:16px;letter-spacing:.2px;margin-bottom:10px;">Start playback</div>
        <div style="color:rgba(0,0,0,.65);font-weight:700;font-size:13px;line-height:1.35;margin-bottom:14px;">
          You opened a bookmark. Where should we start?
        </div>
        <div style="display:flex;flex-direction:column;gap:10px;">
          <button id="bmChoiceContinue" class="bigBtn" style="width:100%;">Continue from last stop</button>
          <button id="bmChoiceBookmark" class="bigBtn secondary" style="width:100%;">Start from bookmark</button>
          <button id="bmChoiceCancel" class="pillBtn" style="width:100%;">Cancel</button>
        </div>
      </div>`;
    document.body.appendChild(wrap);
