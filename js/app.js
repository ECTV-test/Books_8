...
function applyLanguagePairChange(){
  try{
    ...
    try{
      if(state.route?.name === "reader" || state.route?.name === "bireader"){
        setCursorIndex(idx, {syncUI:true, scroll:true});
      // Auto-play when opened from a bookmark
      if(state.route?.autoPlay){
        try{ state.route.autoPlay = false; }catch(e){}
        setTimeout(()=>{ try{ if(!state.reading.isPlaying) startReading(); }catch(e){} }, 60);
      }
      }
    }catch(e){}
  }catch(e){}
...
function _bmOpenFromEntry(bookId, entry, play){
  ...
  go({name: routeName, bookId: bookId, startIndex: idx, forceStartIndex: true, autoPlay: (mode==="listen")});
}
...
function renderLibrary(){
  ...
  if(__btb){
    __btb.onclick = ()=>{
      ...
      go({name: routeName, bookId: ctx.bookId, startIndex: Number(ctx.lineIndex||0), forceStartIndex:true, autoPlay: (String(ctx.mode||"read")==="listen")});
    };
  }
  ...
  if(tab==="bookmarks"){
    app.querySelectorAll("[data-bm-play]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        if(it) _bmOpenFromEntry(bookId, it, true);
      });
    });
    app.querySelectorAll("[data-bm-go]".forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        const list = loadBookmarks(bookId);
        const it = list.find(x=>x && x.id===entryId);
        if(it) _bmOpenFromEntry(bookId, it, false);
      });
    });
...
async function showLineCard(paraIdx){
  ...
  try{ popCtx = { bookId: b.id || state.route?.bookId, paraIdx: Number.isFinite(paraIdx) ? Number(paraIdx) : 0,
    lineIndex: Number.isFinite(lineIndex) ? Number(lineIndex) : (Number.isFinite(paraIdx)?Number(paraIdx):0),
    level: String(level||"original"),
    sourceLang: String(sourceLang||"").trim().toLowerCase(),
    targetLang: String(targetLang||"").trim().toLowerCase(),
    mode: String(mode||"read"), raw, tr: "" }; }catch(e){ popCtx = { bookId: b.id || state.route?.bookId, paraIdx: 0, raw, tr: "" }; }
