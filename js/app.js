...
/* --- Bookmark markers (use existing classes, no duplicates) --- */
/* Line bookmark: add small "shelf" on the line button */
.bmLineMark .lineCardBtn{ position:relative; overflow:visible; }
.bmLineMark .lineCardBtn::after{
  content:"";
  position:absolute;
  right:8px;
  bottom:6px;
...
body[data-theme="night"] .bmLineMark .lineCardBtn::after{
  ...
  box-shadow: 0 0 0 1px rgba(255,255,255,.18), 0 1px 0 rgba(0,0,0,.35) inset;
}

/* Word bookmark: visible dot above the word */
.bmWordMark{ position:relative; }
.bmWordMark::before{
  content:"";
  position:absolute;
  left:50%;
...
/* --- Matte glass for text blocks (keep light subtle) --- */
body:not([data-theme="night"]) .paraLine,
body:not([data-theme="night"]) .listenLine{
  background: rgba(255,255,255,.52);
  border: 1px solid rgba(20,24,28,.06);
  box-shadow: none;
}
body[data-theme="night"] .paraLine,
body[data-theme="night"] .listenLine{
  background: rgba(255,255,255,.08);
  border: 1px solid rgba(255,255,255,.08);
}

/* --- Listen light theme "card" underlay (was missing) --- */
body:not([data-theme="night"]) .listenLine{
  -webkit-backdrop-filter: blur(14px) saturate(120%);
  backdrop-filter: blur(14px) saturate(120%);
}

/* --- Top bars in Read & Listen should be frosted like player bar (all themes) --- */
