...
if(route.name === "details"){
  ...
  if(route.name === "reader"){
    ...
    loadBook(route.bookId, state.reading.sourceLang).then(book=>{
      ...
      // If caller forces startIndex (e.g., bookmarks/chapters), override saved progress
      if(state.route && state.route.forceStartIndex && Number.isFinite(__fallbackStart)){
        idx = __fallbackStart;
      }
try{ clearAllWordHighlights(); }catch(e){}
      setCursorIndex(idx, {syncUI:true, scroll:true});      try{
        if(state.route && state.route.autoPlay){ startReading(); }
      }catch(e){}
