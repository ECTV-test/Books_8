...
if(popBookmark){
  ...
  try{
    ...
    if(allBtn){ allBtn.textContent = t("btn_all"); allBtn.title = t("all_bookmarks_title"); }
  }catch(e){}
...
function renderChaptersList(){
  ...
  // progress: mark completed + highlight current chapter
  let curLine = 0;
  try{ curLine = getCursorIndex(); }catch(e){}
  let curCh = 0;
  try{
    for(let i=0;i<ch.length;i++){
      const si = Number(ch[i]?.startIndex);
      if(Number.isFinite(si) && si<=curLine) curCh = i;
    }
  }catch(e){}

  chaptersList.innerHTML = ch.map((c, idx)=>{
    const title = _escHtml(String(c.title||"Chapter"));
    const isActive = idx===curCh;
    const isDone = idx<curCh;
    return `<button class="btn" style="text-align:left;justify-content:flex-start;gap:10px;${isActive?'background:#dbeafe;':''}" data-chapter="${idx}">
      ...
      ${isDone?`<span style="font-weight:900;opacity:.65">✓</span>`:''}
                ...
                ${pkgs && pkgs.length ? `
                  <div class="pkgRow">
                    ${pkgs.map(x=>{
                      ...
                      return `<span class="pkgChip ${isResume?'resume':''}" data-resume="${resumeKey}" role="button" tabindex="0" title="Resume"><span class="lvl">${lead}</span> ${flagFor(s)} ${s.toUpperCase()} <span class="arrow">→</span> ${flagFor(trg)} ${trg.toUpperCase()} <span class="mode">${modeLabel}</span> <span class="pct">${pct}%</span></span>`;
                    }).join("")}
                  </div>
                ` : ``}
              </div>
