...
function renderCatalog(){
  ...
  try{
    if(cont){
      ...
      if(last){
        const lp = getPkgProgress(cont.id, last.sourceLang, last.targetLang, normalizeLevel(last.level||"original"));
        if(lp && typeof lp.progress === "number"){
          contShowPct = Number(lp.progress||0);
          contShowLabel = `${(normalizeLevel(last.level||"original")==="original")?"Original":String(normalizeLevel(last.level||"original")).toUpperCase()} • ` + formatPkgLabel(last.sourceLang, last.targetLang, last.mode);
        }
      }
    }
  }catch(e){}
      ...
      ${cont ? `
        ...
        <div class="cardWide" id="continueCard" role="button" tabindex="0">
          ...
          <div class="info">
            ...
            <p class="meta">${escapeHtml((cont.author||cont.series||"")||"")}${contShowLabel?` • ${escapeHtml(contShowLabel)}`:``}</p>
          </div>
              ...
              <div style="flex:1; min-width:0;">
                ...
                ${pkgs && pkgs.length ? `
                  <div class="pkgRow">
                    ${(()=>{ 
                      const MAX_CHIPS = 3;
                      const shown = pkgs.slice(0, MAX_CHIPS);
                      const hidden = Math.max(0, pkgs.length - shown.length);
                      const html = shown.map(x=>{
                        ...
                        const lead = isResume ? `▶ Resume: ${lvLabel}` : lvLabel;
                      ...
                      const more = hidden>0 ? `<span class="pkgChip more">+${hidden}</span>` : "";
                      return html + more;
                    })()}
                  </div>
                ` : ``}
...
function renderDetails(){
  ...
  const pagesEst = (()=>{
    const totalLines = Number(b.text?.length||0);
    if(b.pages){ return Number(b.pages)||1; }
    // Virtual pages: depends on current font size (bigger font => fewer lines per page)
    const baseFont = 22;
    const baseLpp = 14;
    const fs = Number(state.reading?.fontSize||baseFont) || baseFont;
    const lpp = Math.max(8, Math.round(baseLpp * (baseFont / fs)));
  ...
  // header: last active package line
  const lastPkg = getLastPkg(b.id);
  const lastLevel = normalizeLevel(lastPkg?.level||"original");
  const lastLevelLabel = (lastLevel==="original") ? "Original" : String(lastLevel).toUpperCase();
  const headerLine3 = (()=>{
    const pct = Math.round(Number(savedPct||0));
    const pkg = savedLabel ? savedLabel : "";
    // Third line under title: • Level • ~pages • % • Pair (Mode)
    return `• ${lastLevelLabel} • ~${pagesEst} pages • ${pct}%${pkg?` • ${pkg}`:""}`;
  })();
    ...
    <div class="detailsGrid">
      ...
      <div>
        <div>
          ...
          <p class="detailsMeta1">${escapeHtml((b.author||b.series||"")||"")}</p>
          <p class="detailsMeta2">${escapeHtml(headerLine3)}</p>
