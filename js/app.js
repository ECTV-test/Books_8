...
const playNext = async ()=>{
    ...
    try{
      ...
      console.warn(err);
      const nextIdx = idx + 1;
      setCursorIndex(nextIdx, {syncUI:false});
      // keep UI indices in sync (without DOM work) so observers can't snap cursor back
      try{
        if(mode === "bireader") state.reading.activeBiLineIndex = nextIdx;
        else state.reading.activeParaIndex = nextIdx;
      }catch(e){}
      try{ saveReadingProgress(); }catch(e){}
...
if(mode === "reader" && !speakTr && state.reading.highlight){
  ...
  if(Number.isFinite(openaiAudio.duration) && openaiAudio.duration > 0) startHL();
    ...
    openaiAudio.onended = ()=>{
      ...
      if(mode === "bireader") state.reading.resumeIndexBi = nextIdx; else state.reading.resumeIndexReader = nextIdx;
      try{ saveReadingProgress(); }catch(e){}
      playNext();
    };
    openaiAudio.onerror = ()=>{
