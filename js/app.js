...
function addBookmarkFromPopover(){
  try{
    ...
    addBookmarkEntry({bookId, paraIdx: (popCtx.paraIdx||0), raw, tr, lineIndex: (popCtx.lineIndex ?? popCtx.paraIdx ?? 0), level: (popCtx.level || state.reading?.level || "original"), sourceLang: (popCtx.sourceLang || sLang), targetLang: (popCtx.targetLang || tLang), mode: __mode, wordIndex: __wi, wordKey: __wk});
...
function removeBookmarkEntry(bookId, entryId){
  ...
  const list = loadBookmarks(bookId).filter(x=>x && x.id !== ent
