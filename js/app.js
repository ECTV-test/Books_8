...
/* Short description for Library subtitles (future: author/series/category from metadata) */
function getBookSubtitle(book){
  if(!book) return "";
  // explicit fields first (future-ready)
  const direct = (book.blurb || book.tagline || book.subtitle || "").toString().trim();
  if(direct) return truncateText(direct, 92);

  const desc = (book.description || "").toString();
  if(!desc) return "";

  // remove title / level lines and empty lines
  const lines = desc.split(/\n/).map(x=>String(x||"").trim()).filter(Boolean);
  const cleaned = [];
  for(const ln of lines){
    if(cleaned.length===0){
      // skip if equals title
      const t = String(book.title_en || book.title_ua || "").trim().toLowerCase();
      if(t && ln.toLowerCase()===t) continue;
    }
    if(/^(рівень|уровень|level)\s*[-:]/i.test(ln)) continue;
    cleaned.push(ln);
              ...
              ${items.map(b=>`
                <div class="bookTile" data-open="${escapeHtml(b.id)}">
                  ...
                  <div class="tileMeta">
                    ...
                    <p class="tileSub">${escapeHtml(getBookSubtitle(b))}</p>
                  </div>
                </div>
              `).join("")}
            </div>
          ...
          (list.length ? list.map(({b,p,pkgs})=>`
            <div class="libraryItem" data-open="${escapeHtml(b.id)}">
              ...
              <div style="flex:1; min-width:0;">
                ...
                <p class="meta">${escapeHtml(getBookSubtitle(b))}</p>
                ${pkgs && pkgs.length ? `
                  <div class="pkgRow">
                    ${pkgs.map(x=>{
                      ...
                      const lead = isResume ? "▶ Resume:" : lvLabel;
