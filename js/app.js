...
function renderCatalog(){
  ...
  if(cont){
    ...
    contMeta1 = _author ? `${_author} • ${_series}` : _series;
    contMeta2 = ""; // will be filled with progress label below
              ...
              <div class="bmHead" data-open="${escapeHtml(b.id)}">
                ...
                <div style="flex:1; min-width:0;">
                  <div class="bmTitleRow">
                    ...
                    <div class="bmMeta">${escapeHtml((String((b.author||"")||"").trim()?String((b.author||"")||"").trim()+" • ":"") + (String((b.series||"")||"").trim()||""))}</div>
...
function injectUiPatchV3(){
  try{
    ...
    st.id = "uiPatchV3";
    st.textContent = `
/* --- Glass + theme fixes (v3) --- */

/* Make top header bar glass like player bar (both themes) */
.readTopBar{
  position: sticky;
  top: 0;
  z-index: 50;
...
body.night .readTopBar{
  background: rgba(10,18,30,.55);
  border-bottom: 1px solid rgba(255,255,255,.08);
}

/* Give "glass" feel to listen lines and chapter sheets */
.listenLine{
  backdrop-filter: blur(10px) saturate(135%);
  -webkit-backdrop-filter: blur(10px) saturate(135%);
  background: rgba(255,255,255,.55);
}
body.night .listenLine{ background: rgba(10,18,30,.45); }

/* Restore LISTEN light-mode line background if it vanished */
body:not(.night) .listenLine{ background: rgba(255,255,255,.62); }

/* Bookmarks sheet/list items: use glass instead of flat grey */
.bmSheet, .bmSheetWrap, .bmSheetMain{
  background: transparent !important;
}
.bmSheetTop, .bmSheetTitle, .bmSheetActions{
  color: inherit;
}
.bmSheetItem{
...
body.night .bmSheetItem{
  background: rgba(10,18,30,.45);
  border: 1px solid rgba(255,255,255,.08);
}

/* Bookmark markers visibility (dots over word, bar under line button) */
.w.bmWordMark{
  position: relative;
...
body.night .w.bmWordMark{ background: rgba(120,200,255,.22); }
.w.bmWordMark::before{
  content:"";
  position:absolute;
  left:50%;
  ...
  border-radius:999px;
  background: rgba(66,153,225,.95);
  box-shadow: 0 2px 8px rgba(66,153,225,.35);
}
body.night .w.bmWordMark::before{
  background: rgba(180,230,255,.95);
  box-shadow: 0 2px 10px rgba(180,230,255,.35);
}

/* Line bookmark indicator on the right "lineCardBtn" */
.listenLine.bmLineMark{ position:relative; }
.listenLine.bmLineMark .lineCardBtn{
  position:relative;
}
.listenLine.bmLineMark .lineCardBtn::after{
  content:"";
  position:absolute;
  left:50%;
  transform:translateX(-50%);
...
body.night .listenLine.bmLineMark .lineCardBtn::after{
  background: rgba(180,230,255,.95);
}

/* Make the bookmark button itself visible in night (top bar) */
body.night .topIcon, body.night .chevBtn{
  color: rgba(255,255,255,.92) !important;
  background: rgba(255,255,255,.08) !important;
  border: 1px solid rgba(255,255,255,.10) !important;
}

/* Library top titles readability on night */
body.night .bigTitle, body.night .bigTitleMuted, body.night .libTitle{
  color: rgba(255,255,255,.92) !important;
}
body.night .bigTitleMuted{ color: rgba(255,255,255,.35) !important; }

/* Small meta line in bookmark book headers */
.bmMeta{
  margin-top: 2px;
  font-size: 13px;
  font-weight: 600;
...
body.night .bmMeta{ opacity: .85; }

/* Remove "extra frame" feeling in light mode by toning borders */
body:not(.night) .bmSheetItem{ border-color: rgba(0,0,0,.04); }
    `;
