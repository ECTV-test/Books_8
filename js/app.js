  ...
  if(tab==="bookmarks"){
    ...
    app.querySelectorAll("[data-bm-go]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        const list = loadBookmarks(bookId);
        const it = list.find(x=>x && x.id===entryId);
        // apply bookmark context (level + langs) if present
        if(it){
          try{ if(it.level) state.reading.level = String(it.level); }catch(e){}
          try{ if(it.sourceLang) state.reading.sourceLang = String(it.sourceLang); }catch(e){}
          try{ if(it.targetLang) state.reading.targetLang = String(it.targetLang); }catch(e){}
        }
        const idx2 = (Number.isFinite(it?.lineIndex)?Number(it.lineIndex):Number(it?.paraIdx||0));
        // open in Listen mode and force jump to bookmark line
        go({name:"reader", bookId, startIndex: idx2, forceStartIndex:true, autoPlay:false});
      });
    });
