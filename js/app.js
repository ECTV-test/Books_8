...
function formatLevelLabel(code){
  ...
  if(c.toLowerCase()==="original") return "ORIG";
  ...
  wrap.innerHTML = `
    <div class="bmSheet">
      ...
      <div class="bmSheetList">
        <div class="bmGroupHeader" data-bm-group-resume="${escapeHtml(bookId)}|${escapeHtml(String(ctxLevel||"original"))}|${escapeHtml(String((ctxPair&&ctxPair.src)||"en"))}|${escapeHtml(String((ctxPair&&ctxPair.trg)||"uk"))}|${escapeHtml(String(ctxMode||"read"))}" title="Resume">
          <span>${escapeHtml(formatLevelLabel(ctxLevel||"original"))} <span class="bmSep">•</span> ${escapeHtml(formatPkgLabel((ctxPair&&ctxPair.src)||"en", (ctxPair&&ctxPair.trg)||"uk", ctxMode||"read"))}</span>
          <span class="hint">↪︎</span>
        </div>
              ...
              <div class="bmHead" data-open="${escapeHtml(b.id)}">
                ...
                <div style="flex:1; min-width:0;">
                  <div class="bmTitleRow">
                    ...
                    ${tab==="bookmarks" && state.ui?.backToBook && state.ui.backToBook.bookId===b.id ? ` <button class="bmBackMini inline" id="backToBookBtn" title="Back to book">↩︎ Back</button>` : ``}
                  </div>
                </div>
                              </div>
              <div class="bmItems">
                ...
                ${pkgs && pkgs.length ? `
                  <div class="pkgRow">
                    ${pkgs.map(x=>{
                      ...
                      const lead = lvLabel;
                      const resumeKey = `${escapeHtml(b.id)}|${escapeHtml(lv)}|${escapeHtml(s)}|${escapeHtml(t)}|${escapeHtml(m)}`;
                      return `<span class="pkgChip ${isResume?'resume':''}" data-resume="${resumeKey}" role="button" tabindex="0" title="Resume"><span class="lvl">${lead}</span> ${flagFor(s)} ${s.toUpperCase()} <span class="arrow">→</span> ${flagFor(t)} ${t.toUpperCase()} <span class="mode">${modeLabel}</span> <span class="pct">${pct}%</span></span>`;
                    }).join("")}
                  </div>
                ` : ``}
              </div>
