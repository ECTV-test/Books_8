              ...
              <div class="bmItems">
                ${items.map((it, idx)=>`
                  <div class="bmItem" data-bm-item>
                    <div class="bmMain">
                      <p class="bmLabel">Закладка ${idx+1} <span class="bmSep">•</span> <span class="bmLevel">${escapeHtml(formatLevelLabel(it.level||"original"))}</span> <span class="bmSep">•</span> <span class="bmPair">${escapeHtml(formatPkgLabel((it.sourceLang||getLastPkg(b.id)?.sourceLang||state.reading?.sourceLang||"en"), (it.targetLang||getLastPkg(b.id)?.targetLang||state.reading?.targetLang||"uk"), (it.mode||getLastPkg(b.id)?.mode||"read")) )}</span></p>
