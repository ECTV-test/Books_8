...
function addBookmarkFromPopover(){
  try{
    ...
    addBookmarkEntry({bookId, paraIdx: popCtx.paraIdx || 0, lineIndex: (Number.isFinite(popCtx.lineIndex)?popCtx.lineIndex:(popCtx.paraIdx||0)), wordIdx: (Number.isFinite(popCtx.wordIdx)?popCtx.wordIdx:-1), raw, tr, level: popCtx.level || normalizeLevel(state.reading?.level||"original"), sourceLang: sLang, targetLang: tLang, mode: m});
...
function jumpToChapter(chIdx){
  ...
  if(state.route?.name === 'reader' || state.route?.name === 'bireader'){
    ...
    setCursorIndex(idx, {syncUI:true, scroll:true});
      // Bookmark: highlight specific word (if provided)
      try{
      ...
      // IMPORTANT: clear one-time navigation hints so they don't affect later actions
  ...
  if(tab==="bookmarks"){
    ...
    app.querySelectorAll("[data-bm-go]").forEach(btn=>{
      btn.addEventListener("click", (e)=>{
        ...
        go({name:"reader", bookId, startIndex: idx, forceStartIndex:true, startWordIndex: (Number.isFinite(it?.wordIdx)?Number(it.wordIdx):-1)});
      });
    });
...
async function showTranslation(span){
  ...
  try{
    ...
    const srcLang = String(state.reading?.sourceLang || "en").trim().toLowerCase();
    const trgLang = String(state.reading?.targetLang || "uk").trim().toLowerCase();
    const lvl = normalizeLevel(state.reading?.level || "original");
    const mde = pkgMode(state.route?.name);
    // compute word index within the paragraph
    let wi = -1;
    try{
      const arr = state.reading?.paraWords?.[pIdx];
      if(Array.isArray(arr)) wi = arr.indexOf(span);
    }catch(e){}
    popCtx = {
      bookId: state.book?.id || state.route?.bookId,
      paraIdx: Number.isFinite(pIdx) ? pIdx : 0,
      lineIndex: Number.isFinite(pIdx) ? pIdx : 0,
      wordIdx: Number.isFinite(wi) ? wi : -1,
      level: lvl,
      sourceLang: srcLang,
      targetLang: trgLang,
      mode: mde,
    ...
    popCtx = { bookId: state.book?.id || state.route?.bookId, paraIdx: 0, lineIndex:0, wordIdx:-1, raw, tr:"" };
  }
...
async function showLineCard(paraIdx){
  ...
  try{
    const bookId = b.id || state.route?.bookId || state.book?.id;
    const srcLang = String(state.reading?.sourceLang || "en").trim().toLowerCase();
    const trgLang = String(state.reading?.targetLang || "uk").trim().toLowerCase();
    const lvl = normalizeLevel(state.reading?.level || "original");
    const mde = pkgMode(state.route?.name);
    ...
    popCtx = { bookId: b.id || state.route?.bookId, paraIdx: Number.isFinite(paraIdx)?Number(paraIdx):0, raw, tr: "" };
  }
