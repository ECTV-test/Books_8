...
(function(){
  ...
  window.createCoreV8 = function createCoreV8(opts){
    ...
    function loadProgress(bookId, src, trg, level){
      ...
      // IMPORTANT:
      // Legacy (v7) progress is shared across levels.
      // In v8, levels must NOT inherit progress from each other.
      // Therefore we only migrate legacy progress into v8 when level is "original".
      if(String(lv).toLowerCase() !== "original"){
        return null;
      }

      // migrate from legacy shared pkg (original only)
    ...
    function setLevel(level){
      ...
      switchLangPair(state.src, state.trg, level);
    }
...
function getLastPkg(){
    ...
    function buildChaptersFromLines(lines){
      ...
      for(let i=0;i<lines.length;i++){
        ...
        // heuristic: short standalone line between blanks
        const prevBlank = (i===0) ? true : isBlank(lines[i-1]);
        const nextBlank = (i===lines.length-1) ? true : isBlank(lines[i+1]);
        if(prevBlank && nextBlank){
          const len = s.length;
          const looksLikeTitle = (len>=2 && len<=60 && !/[.?!…]$/.test(s)) || (len<=80 && isAllCaps(s));
          if(looksLikeTitle){
            out.push({ index:i, title:s });
          }
        }
      ...
      for(const c of out){
        ...
        seen.add(k);
        uniq.push({ index:k, title:String(c.title||"Chapter") });
      }
    ...
    function getChapters(){
      return Array.isArray(state.chapters) ? state.chapters.slice() : [];
    }
